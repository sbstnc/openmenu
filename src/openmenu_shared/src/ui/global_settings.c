...
/* Images and such */
#if __has_include("ui/openmenu_lcd.h") && __has_include("ui/openmenu_pal.h") && __has_include("ui/openmenu_vmu.h")
#include "ui/openmenu_lcd.h"
#include "ui/openmenu_pal.h"
#include "ui/openmenu_vmu.h"
    ...
    if (savefile_details.valid_memcards) {
        for (int iter = 0; iter <= 3; iter++) {
            for (int jiter = 1; jiter <= 2; jiter++) {
                if (crayon_savefile_get_vmu_bit(savefile_details.valid_memcards, iter,
                                                jiter)) { // Use the left most VMU
...
settings_init(void) {
    ...
    crayon_savefile_init_savefile_details(&savefile_details, (uint8_t*)&savedata, sizeof(openmenu_settings),
                                          OPENMENU_ICONS, 1, "openMenu Preferences\0", "openMenu Config\0",
                                          "openMenuPref\0", "OPENMENU.CFG\0");
    ...
    for (int iter = 0; iter <= 3; iter++) {
        for (int jiter = 1; jiter <= 2; jiter++) {
            if (crayon_savefile_get_vmu_bit(savefile_details.valid_saves, iter,
                                            jiter)) { // Use the left most VMU
                savefile_details.savefile_port = iter;
                savefile_details.savefile_slot = jiter;
                goto Exit_loop_1;
